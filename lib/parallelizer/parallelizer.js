...
module.exports = function () {
    ...
    eventEmitter.on('initialize', function () {
        ...
        let dirs = Finder.from(cucumberProgram.args[0]).findDirectories();
        dirs.push(cucumberProgram.args[0]);
        for (let dir of dirs) {
            let files = Finder.in(dir).findFiles('*.feature');
    ...
    function forkRunner(options) {
        ...
        var argv_ = _.map(_argv, function(val) {
            return val === pathToFeatures ? options.feature.shift() : val;
        });

        argv_ = argv_.concat(options.feature);
